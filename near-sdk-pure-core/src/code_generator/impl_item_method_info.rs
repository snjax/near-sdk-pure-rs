        ...
        if has_input_args {
            ...
            let serializer_invocation = match attr_signature_info.input_serializer {
                SerializerType::JSON => quote! {
                    near_sdk_pure::serde_json::from_slice(
                        &near_sdk_pure::env::input().expect("Expected input since method has arguments.")
                    ).expect("Failed to deserialize input from JSON.")
                },
                SerializerType::Borsh => quote! {
                    near_sdk_pure::borsh::BorshDeserialize::try_from_slice(
                        &near_sdk_pure::env::input().expect("Expected input since method has arguments.")
        ...
        let deposit_check = if *is_payable || *is_view {
            ...
            quote! {
                if near_sdk_pure::env::attached_deposit() != 0 {
                    near_sdk_pure::env::panic(#error.as_bytes());
                }
            }
        };
        ...
        let body = if *is_init {
            ...
            if let Some(receiver) = receiver {
                ...
                contract_deser = quote! {
                    let #mutability contract: #struct_type = near_sdk_pure::env::state_read().unwrap_or_default();
                };
            ...
            match returns {
                ...
                ReturnType::Type(_, _) => {
                    let value_ser = match result_serializer {
                        SerializerType::JSON => quote! {
                            let result = near_sdk_pure::serde_json::to_vec(&result).expect("Failed to serialize the return value using JSON.");
                        },
                        SerializerType::Borsh => quote! {
                            let result = near_sdk_pure::borsh::BorshSerialize::try_to_vec(&result).expect("Failed to serialize the return value using Borsh.");
                        },
                    };
                    quote! {
                    #contract_deser
    ...
    pub fn marshal_method(&self) -> TokenStream2 {
        ...
        let json_args = if has_input_args {
            let args: TokenStream2 = attr_signature_info
                ...
                .unwrap();
            quote! {
              let args = near_sdk_pure::serde_json::json!({#args});
            }
        } else {
            quote! {
             let args = near_sdk_pure::serde_json::json!({});
            }
        };
        ...
        let return_ident = quote! { -> near_sdk_pure::PendingContractTx };
        let params = quote! {
            &self, #pat_type_list
        };
        let ident_str = format!("{}", ident.to_string());
        let body = if *is_view {
            quote! {
                near_sdk_pure::PendingContractTx::new(&self.account_id, #ident_str, args, true)
            }
        } else {
            quote! {
                near_sdk_pure::PendingContractTx::new(&self.account_id, #ident_str, args, false)
            }
        };
        let non_bindgen_attrs = non_bindgen_attrs.iter().fold(TokenStream2::new(), |acc, value| {
            quote! {
