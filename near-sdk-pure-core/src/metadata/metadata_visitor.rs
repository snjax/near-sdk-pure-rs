    ...
    pub fn generate_metadata_method(&self) -> syn::Result<TokenStream2> {
        ...
        Ok(quote! {
            ...
            pub extern "C" fn metadata() {
                #env_creation
                use borsh::*;
                let metadata = near_sdk_pure::Metadata::new(vec![
                    #(#methods),*
                ]);
                let data = near_sdk_pure::borsh::BorshSerialize::try_to_vec(&metadata).expect("Failed to serialize the metadata using Borsh");
                near_sdk_pure::env::value_return(&data);
            }
        })
    }
}
    ...
    fn several_methods() {
        ...
        let expected = quote!(
            ...
            pub extern "C" fn metadata() {
                ...
                let data = near_sdk_pure::borsh::BorshSerialize::try_to_vec(&metadata)
